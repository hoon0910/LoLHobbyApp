...
class ChampionViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun fetchChampionData() {
        viewModelScope.launch {
            try {
                // 최신 버전 가져오기
                val versionResponse = NetworkManager.dataDragonApiService.getVersions()
                if (versionResponse.isSuccessful) {
                    ...
                    if (!preVer.equals(version)) {
                        val championResponse = NetworkManager.dataDragonApiService.getChampionData(version)
    ...
    fun addOrUpdateChampion(id: Int, name: String) {
        // Update the map and emit a new state
    ...
    fun fetchChampionRotation() {
        viewModelScope.launch {
            try {
                val response = NetworkManager.riotApiService.getChampionRotation()
